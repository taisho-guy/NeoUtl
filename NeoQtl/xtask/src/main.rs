...
fn stage_plugin_host(
    ...
    let dest_dir = target_dir(dest_root, profile, target);
...
fn stage_scripts(src_root: &Path, dest_root: &Path, profile: &str, target: Option<&str>) {
    let src_dir = src_root.join("scripts");
    if !src_dir.is_dir() {
        return;
    }
    let dst_dir = target_dir(dest_root, profile, target).join("scripts");
    copy_dir_recursive(&src_dir, &dst_dir).expect("Luaスクリプト配置失敗");
    eprintln!("[xtask] 配置: scripts/（Lua）");
}

fn stage_themes(src_root: &Path, dest_root: &Path, profile: &str, target: Option<&str>) {
    let src_dir = src_root.join("assets").join("themes");
    if !src_dir.is_dir() {
        return;
    }
    let dst_dir = target_dir(dest_root, profile, target).join("themes");
    copy_dir_recursive(&src_dir, &dst_dir).expect("テーマ配置失敗");
    eprintln!("[xtask] 配置: themes/");
}
...
fn neoqtl_root() -> PathBuf {
    workspace_root().join("NeoQtl")
}
...
fn main() {
    ...
    stage_plugin_host(&root, &neoqtl, profile, target, offline);
    stage_crates(&neoqtl, profile, target, "objects", &objects);
    stage_crates(&neoqtl, profile, target, "effects", &effects);
    stage_crates(&neoqtl, profile, target, "decoders", &decoders);
    fs::create_dir_all(target_dir(&neoqtl, profile, target).join("easings"))
        .expect("easings配置先ディレクトリ作成失敗");
    stage_scripts(&root, &neoqtl, profile, target);
    stage_themes(&root, &neoqtl, profile, target);

    if task != "run" {
        return;
    }

    let bin_path = target_dir(&neoqtl, profile, target).join(exe_filename("NeoQtl"));
