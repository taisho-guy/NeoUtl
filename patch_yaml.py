with open(".github/workflows/release-windows-x86_64.yml", "r", encoding="utf-8") as f:
    content = f.read()

new_content = """name: Build and Release Windows (x86_64)

on:
  push:
    tags:
      - '**'

permissions:
  contents: write

jobs:
  build-windows:
    name: Windows (MSYS2 UCRT64)
    runs-on: windows-latest
    steps:
      - uses: actions/checkout@v4
      
      - name: Setup MSYS2
        uses: msys2/setup-msys2@v2
        with:
          msystem: UCRT64
          update: true
          install: >-
            mingw-w64-ucrt-x86_64-python
            mingw-w64-ucrt-x86_64-python-pyside6

      - name: Build and Package
        shell: msys2 {0}
        run: python BUILD.py --msys2
        env:
          GIT_TERMINAL_PROMPT: 0

      - name: Upload Artifact
        uses: actions/upload-artifact@v4
        with:
          name: AviQtl-MSYS2-UCRT64-x86_64
          path: dist/AviQtl-MSYS2-UCRT64-x86_64.zip
          retention-days: 1
          if-no-files-found: error

      - name: Create Codeberg Release
        if: startsWith(github.ref, 'refs/tags/')
        uses: akkuman/gitea-release-action@v1
        env:
          GITEA_TOKEN: ${{ secrets.CODEBERG_TOKEN }}
        with:
          server_url: https://codeberg.org
          token: ${{ secrets.CODEBERG_TOKEN }}
          repository: taisho-guy/AviQtl
          draft: false
          prerelease: false
          tag_name: ${{ github.ref_name }}
          name: ${{ github.ref_name }}
          body: ""
          files: |
            dist/AviQtl-MSYS2-UCRT64-x86_64.zip

      - name: Delete GitHub Actions Artifact
        if: success()
        uses: geekyeggo/delete-artifact@v5
        with:
          name: AviQtl-MSYS2-UCRT64-x86_64
"""

with open(".github/workflows/release-windows-x86_64.yml", "w", encoding="utf-8") as f:
    f.write(new_content)
