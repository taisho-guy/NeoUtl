const initReleases = async () => {
    ...
    const PATTERNS = {
        linux: ["linux"],
        windows: ["win", "msys2", "ucrt"],
        apple: ["mac", "apple", "darwin"],
    };
    ...
    const getArch = () => {
        const selected = document.querySelector('.arch-tab[aria-selected="true"][data-arch]')
            ?.dataset?.arch;
        if (selected === "x86_64" || selected === "arm64") return selected;
        ...
        const isX86 =
            ...
            ua.includes("amd64") ||
            ua.includes("x64") ||
            legacyPlatform.includes("x86") ||
            legacyPlatform.includes("amd64") ||
            legacyPlatform.includes("x64");

        if (isArm && !isX86) return "arm64";
        if (isX86 && !isArm) return;
        ("x86_64");
    ...
    const updateUI = (latest, prev) => {
      ...
      target="_blank">過去のバージョン</a>`;
        ...
        const archFilters =
            selectedArch === "arm64" ? ["aarch64", "arm64"] : ["x86_64", "amd64", "x86-64"];

        for (const el of document.querySelectorAll(".download-item[data-platform]")) {
            const p = el.dataset.platform;
            if (p === "source") {
                el.href = `https://codeberg.org/${REPO}/archive/${latest.tag_name}.zip`;
                continue;
            }

            const asset = latest.assets.find((a) => {
                const name = (a.name ?? "").toLowerCase();
                const osMatch = PATTERNS[p]?.some((s) => name.includes(s));
                if (!osMatch) return false; //
                const hasArchHint =
                    archFilters.some((f) => name.includes(f)) ||
                    (selectedArch === "arm64" && (name.includes("aarch") || name.includes("arm")));
                if (!hasArchHint) return true;
                return archFilters.some((f) => name.includes(f));
            });
        ...
        document
            .querySelectorAll(".download-icon")
            .forEach((i) => i.classList.remove("is-loading"));
    };

    // 初期選択（タブが未選択なら推定を反映） const
    archTabs = document.getElementById("arch-tabs");
    if (archTabs) {
        const initial = getArch();
        setArchTabs(initial);
        archTabs.addEventListener("click", (e) => {
            const btn = e.target.closest?.(".arch-tab[data-arch]");
            if (!btn) return;
            setArchTabs(btn.dataset.arch); //
            const cached = localStorage.getItem(CACHE_KEY);
            if (cached) {
                const { latest, prev } = JSON.parse(cached);
                if (latest) updateUI(latest, prev);
            }
        });
    }
                ...
                try {
                    ...
                    if (latest) {
                        localStorage.setItem(
                            CACHE_KEY,
                            JSON.stringify({ latest, prev, timestamp: Date.now() }),
                        );
